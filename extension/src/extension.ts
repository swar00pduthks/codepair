...
export function activate(context: vscode.ExtensionContext) {
    ...
    collaborationManager = new CollaborationManager(context);
    ...
    let statusCommand = vscode.commands.registerCommand('codepair.showStatus', () => {
        if (collaborationManager.isActive()) {
            ...
            const participantList = participants.map(p => 
                `${p.name}${p.githubUsername ? ` (@${p.githubUsername})` : ''}`
            ).join('\n');

            vscode.window.showInformationMessage(
                `Active collaboration in room: ${roomId}\n\nParticipants:\n${participantList}`,
                'Copy Room ID',
                'Stop Collaboration'
            ).then(selection => {
    ...
    let participantsCommand = vscode.commands.registerCommand('codepair.showParticipants', () => {
        if (collaborationManager.isActive()) {
            ...
            const panel = vscode.window.createWebviewPanel(
                'codepairParticipants',
                'CodePair Participants',
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('codepair.copyRoomId', () => {
            const roomId = collaborationManager.getRoomId();
            if (roomId) {
                vscode.env.clipboard.writeText(roomId);
                vscode.window.showInformationMessage('Room ID copied to clipboard!');
    ...
    vscode.window.showInformationMessage(
        'CodePair extension activated! Use Ctrl+Shift+P and search for "CodePair" to start collaborating.',
        'Start Collaboration',
        'Learn More'
    ).then(selection => {
